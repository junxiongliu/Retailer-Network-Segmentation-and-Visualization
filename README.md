...
* Data Visualization:<br/>
	* Go into `scripts/viz` folder and copy all data files under `data/generated` folder to 'scripts/viz/data'. You can use one command to do this. For Windows cmd users, type `Xcopy ..\..\data\generated data\` and for Linux cmd, type `cp -r ../../data/generated/. data/`.
	* Type `python -m http.server` in consle/cmd. Afterwards open up your browser and type `localhost: 8000` (or whatever default port you set up) and you should be able to see the visualization exactly like the screenshot above.
